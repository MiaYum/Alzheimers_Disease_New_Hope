...
We used data from 2018 to 2020 to train our models, and data from 2021 to perform prediction.
...
Despite all the varibles are significant, $R^2$ value is around 0.4, which is suggests potential nonlinear relationships or multicollinearity issues in our dataset.

# Random Forest Model

## Model Selection

The Random Forest Model is an ensemble learning method, that plays an important role in classification and regression tasks by building multiple decision trees and aggregating their outputs. It handles complex datasets well and is robust against overfitting. Thus, we consider switching to Random Forest when linear models have limitations. It effectively captures complex, non-linear relationships in data, offering improved accuracy and deeper insights.
...
We also use cross-validation and bootstrap to see the performance and reliability of the random forest model as we did to the linear model.
...
The bootstrap results of the Random Forest model showing an $R^2$ value around 0.96 indicate a high level of accuracy and consistency. This suggests that the model is not only effective in explaining a large portion of the variance in the data but also demonstrates robust performance across different samples.
