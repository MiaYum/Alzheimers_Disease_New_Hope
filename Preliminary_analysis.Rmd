...
* The table shows the arranged average death rate of Alzheimer’s Disease of each region from 2018 to 2021.

* South region has the highest average Alzheimer’s Disease death rate while Northeast region has the lowest.
...
<center> *H~0~ : mean death_rate group 1 = mean death_rate group 2  vs  H~1~ :  mean death_rate group 1 ≠ mean death_rate group 2*<center>
 
```{r, message = FALSE, warning = FALSE}
aov(death_rate ~ factor(income_group), data = income_level) %>%
  broom::tidy() %>%
  knitr::kable()
```
...
In this part, we applied statistical analysis to studying the differences in  Alzheimer's Disease death rate caused by various factors. We first examine the distribution features and then choose appropriate tests to perform.
We draw the following conclusions from the tests and plots result :
* South region has the highest Alzheimer's Disease average death rate while Northeast region has the lowest.
* 

Regional differences in Alzheimer’s Disease death rates were observed, with significant variations among income and employment groups.
Income and employment status were associated with Alzheimer’s Disease death rates, with lower income and employment linked to higher death rates.
These findings suggest that regional, income, and employment factors play a role in Alzheimer’s Disease death rates, providing valuable insights for further investigation and public health interventions.（我感觉写两句话就行
