...
Our research focuses on the complex relationship between Alzheimer's Disease and its risk factors as highlighted in [existing studies](https://www.alzinfo.org/articles/non-genetic-risk-factors/). Key elements include: (1) Age-related brain changes like neuron degeneration and protein build-up; (2) Educational attainment, potentially offering neural protection; (3) Cardiovascular risk factors akin to heart health impacting Alzheimer's risk; (4) The link between diabetes and cognitive decline; (5) Socioeconomic factors influencing lifestyle and healthcare access, thereby affecting Alzheimer's risk.
...
mean(test$death_rate) - median(test$death_rate)
```
...
test = data |>
  select(-state, -year) |>
  drop_na(cardio_rate, diabetes_rate, smoke_rate, obesity_rate)

for (i in variable.names(test)){
  print(i)
  print(cor(test$death_rate, pull(test, i)))
}
